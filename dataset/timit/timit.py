...
class TimitDataset(torch.utils.data.Dataset):
    def __init__(self, root_folder, encoder, subset='TRAIN', ignore_sa=True, transforms=None):
        root = pathlib.Path(__file__).parents[1].joinpath(root_folder).expanduser()
        wavs = list(root.rglob(f'{subset}/**/*.WAV'))
        wavs = sorted(wavs)
        if ignore_sa:
...
def get_normalize_fn(part_name, eps=0.001):
    stats = np.load(pathlib.Path(__file__).parents[1].joinpath(f'timit_dataset\\timit_train_stats.npz'))
...
def get_ASR_dataloaders(timit_root, batch_size):
    ...
    def collate_fn(batch):
        ...
        return (audio, torch.tensor(audio_lengths)), (
            torch.tensor(sentence, dtype=torch.int32), torch.tensor(sentence_lengths))

    subsets = ['TRAIN', 'VAL', 'TEST']
    datasets = [TimitDataset(timit_root, encoder, subset=s, ignore_sa=True, transforms=get_transforms(s)) for s in
                subsets]
    train_sampler = SubsetRandomSampler(datasets[0].get_indices_shorter_than(None))
    loaders = [DataLoader(d, batch_size=batch_size, sampler=train_sampler if not i else None, pin_memory=True,
                          collate_fn=collate_fn) for i, d in enumerate(datasets)]
...
if __name__ == '__main__':
    ...
    train_load, val_load, test_load = get_ASR_dataloaders('TIMIT', 3)

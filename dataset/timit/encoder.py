...
class PhonemeEncoder:
    ...
    def __init__(self, num_classes, remove_folded=True):
        ...
        mapping = pathlib.Path(__file__).parents[1].joinpath(r'timit_dataset\timit_folding.txt').read_text().strip().split('\n')
    ...
    def _fold(self, phonemes, dst_class_idx=None):
        ...
        return [self.mappings[0][dst_class_idx][p] for p in phonemes if
                not self.remove_folded or p not in self.to_delete[0][dst_class_idx]]
    ...
    def encode(self, phonemes):
        ...
        enc = [self.encodeds[self.class_idx].index(p) + 1 if p else 0 for p in
               phonemes_folded]  # start from 1, 0 is used for blank
        return enc
